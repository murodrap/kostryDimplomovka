 ...
 */
long double SpanningTreeCounter::gauss(int n) {
    int r1 = 0;
    int s1 = 0;
    ...
    //long long rrrr = (long long) result;
   //std::cout << rrrr << std::endl;
   //std::cout << std::fixed << result << "    " << std::llround(result) << "   " << llroundl(result);
    return round(result);
 ...
 */
long double SpanningTreeCounter::kofaktor(int n, int r, int s) {
 ...
 */
long double SpanningTreeCounter::countForGraph(const Edges& edges) {
    for (int i= 0; i < n-1; i++) {
